...
[ExtractorType(ExtractorTypes.Rar)]
    ...
    IFileSizeBiggerZeroHashOutputFileCheckerTrait
{
    ...
    public override string? ImageUrl => "/Resources/extractor-rar.png";

    public override string Hint => $"{_translator.Translate("Extractors.RAR")}";
    ...
    public override List<IExtractorStepViewModel> GetRequiredScreens()
    {
        ...
        folderOrFileExtractorStepViewModel.Filter = "RAR Files (*.rar)| *.rar";
    ...
    {
        var exeFilePath = Path.Combine(AppContext.BaseDirectory, @"Packages\JtR\run\rar2john.exe");

        var timestamp = $"{DateTime.Now:ddMMyy_HHmmssfff}";
        var relativePath = $@"_Hashout\RAR_Extraction_{Path.GetFileNameWithoutExtension(filePath)}_{timestamp}.txt";
    ...
    public override void Stop()
    {
        ...
        KillAllRAR2JohnProcesses();
    }

    private void KillAllRAR2JohnProcesses()
    {
        var exeProcess = Process.GetProcessesByName("rar2john");
    ...
    public async Task<bool?> ExtractHashesFromFolderAsync(string folderPath)
    {
        ...
        var files = Directory.GetFiles(folderPath, "*.rar");
