...
[ExtractorType(ExtractorTypes.VeraCryptPartition)]
public class VeraCryptPartitionExtractor : ExtractorBase,
    ...
    IFileSizeBiggerZeroHashOutputFileCheckerTrait
{
    ...
    public override string Name => "VeraCrypt (Partition)";

    public override string? ImageUrl => "/Resources/extractor-veracrypt.png";

    public override string Hint => $"{_translator.Translate("Extractors.VeraCryptPartition")}";


    string _selectedFilePath;

    public VeraCryptPartitionExtractor(
        IIniFile iniFile,
    ...
    public async Task<bool?> ExtractHashesFromFileAsync(string filePath)
    {
        ...
        var relativePath = $@"_Hashout\VeraCrypt_Partition_Extraction_{Path.GetFileNameWithoutExtension(filePath)}_{timestamp}.txt";
