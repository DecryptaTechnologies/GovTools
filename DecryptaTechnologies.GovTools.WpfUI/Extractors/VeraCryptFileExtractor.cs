...
[ExtractorType(ExtractorTypes.VeraCryptFile)]
public class VeraCryptFileExtractor : ExtractorBase,
    ...
    IFileSizeBiggerZeroHashOutputFileCheckerTrait
{
    ...
    public override string Name => "VeraCrypt (File)";

    public override string? ImageUrl => "/Resources/extractor-veracrypt.png";

    public override string Hint => $"{_translator.Translate("Extractors.VeraCryptFiles")}";


    string _selectedFilePath;

    public VeraCryptFileExtractor(
        IIniFile iniFile,
    ...
    public async Task<bool?> ExtractHashesFromFileAsync(string filePath)
    {
        ...
        var relativePath = $@"_Hashout\VeraCrypt_File_Extraction_{Path.GetFileNameWithoutExtension(filePath)}_{timestamp}.txt";
