...
[ExtractorType(ExtractorTypes.ElectrumWallet)]
public class ElectrumWalletExtractor : ExtractorBase,
    ...
    ICanPostProcessHashOutputFileAsync
{
    ...
    public override string Name => "Electrum";

    public override string? ImageUrl => "/Resources/extractor-electrum.png";
    ...
    public async Task<bool?> ExtractHashesFromFileAsync(string filePath)
    {
        ...
        var scriptFilePath = Path.Combine(AppContext.BaseDirectory, @"Packages\JtR\run\electrum2john.py");

        var timestamp = $"{DateTime.Now:ddMMyy_HHmmssfff}";
        var relativePath = $@"_Hashout\Electrum_Extraction_{Path.GetFileNameWithoutExtension(filePath)}_{timestamp}.txt";
